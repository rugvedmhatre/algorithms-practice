...
2. Perform the same process for each pixel that is directly adjacent (pixels that share a side with
...
def floodFill(image: list[list[int]], sr: int, sc: int, color: int) -> list[list[int]]:
    ...
    while q:
        ...
        for dr, dc in directions:
            ...
            if nr in range(0, m) and nc in range(0, n) and image[nr][nc] == ogc:
                q.append((nr, nc))
